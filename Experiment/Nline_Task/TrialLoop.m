function [trialTiming, trialResults] = TrialLoop(stim, points, left_end, right_end, lineLength, lineSZ, jitter, ppc_adjust, win, color, x1, x2, yline, center, winRect, junk, task, start_t, end_ITI, end_consider, end_hold, end_decision, move, slow, wrong, badpress)
...
%     end;
    ...
    if task ==1;
        trialResults = NumLineSlow_Abs([stim(1), stim(2)], points, left_end, right_end, lineLength, lineSZ, jitter, ppc_adjust, win, color, x1, x2, yline, center, winRect, junk, testX, end_decision, move, slow, wrong, badpress); %decision
    end;
    if task ==2;
        trialResults = NumLineSlow_Abs([stim(1), stim(2)], points, left_end, right_end, lineLength, lineSZ, jitter, ppc_adjust, win, color, x1, x2, yline, center, winRect, junk, testX, end_decision, move, slow, wrong, badpress); %decision
    end;
    if task ==3;
        trialResults = ControlSlow_Abs([stim(1), stim(2), stim(3), stim(4)], points, left_end, right_end, lineLength, lineSZ, jitter, ppc_adjust, win, color, x1, x2, yline, center, winRect, junk, testX, end_decision, move, slow, wrong, badpress); %decision
    end;
