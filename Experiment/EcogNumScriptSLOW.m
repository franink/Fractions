...
time_Runs(1,:) ={'Time_Match', 'Time_NumLine'};
...
%     Screen('Flip', win);
    ...
    for ii = 1:length(TestNumMatch) %right now is 5 just to test quickly
        DrawCenteredNum('X', win, color, fixation_t);
    	nMatchResults(ii+1,1:4) = ConsiderSlow(TestNumMatch(ii,:), win, color, task,consider_t);
        Screen('Flip',win);
        nMatchResults(ii+1,5:9) = NumMatchSlow(TestNumMatch(ii,:), win, color,decision_t);
    end
...
%     Screen('Flip', win);
    ...
    for ii = 1:length(TestFracsLine) %ßright now is 5 just to test quickly
        DrawCenteredNum('X', win, color, fixation_t);
        numlineResults(ii+1,1:4) = ConsiderSlow(TestFracsLine(ii,:), win, color, task,consider_t);
        Screen('Flip',win);
        numlineResults(ii+1,5:9) = FractLineRGSlow(TestFracsLine(ii,:), win, 600, 1, color,decision_t); %think about length of line
    end
...
%save results
save(filename, 'nMatchResults', 'numlineResults', 'time_Runs')
ShowCursor;
