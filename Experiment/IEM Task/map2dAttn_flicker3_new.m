% 282 s - 141 TRs (per attn/spatial condition)

function map2dAttn_flicker3 
...
% adapted on 5/23/12 by TCS from map2dAttn_flicker2.m (from scanner laptop)
% added variable difficulty

% on 7/31/12 updated by TCS to correctly wait after nTRs for p.startDur
...
%get subject info
prompt = {'Subject Name','Condition (1 = fix, 2 = checkerboards, 3 = working memory)','Staggered (-1 or 1)?', 'Block number', 'Random seed', 'fMRI','Eye tracking','Fix Contrast','Stim Contrast','WM Distance'};
...
defAns = {'XXX','3','1','1',num2str(s),'1','0','0.5','0.5','1'};
...
if length(box)==length(defAns)
    ...
    p.fixContrastChange = str2num(box{8});
    p.stimContrastChange = str2num(box{9});
    p.wmDifficulty = str2num(box{10}); % WM difficulty is how far, in radians from center of stim, targ & probe are apart (multiples of pi/2)
...
p.targetProbeMinRadDeg = p.radDeg/3; % must have eccentricity of this or greater...
p.targetProbeMaxRadDeg = p.radDeg/8; % ...and this or smaller
...
for b=p.nBlocks
    ...
    disp('waiting...');
    WaitSecs(p.TR/10); % ADDED TCS 4/24/12
    if p.fMRI
    ...
    for t=1:p.nTrials
        ...
        if ~isempty(p.actualRespFrm(t).d)
            p.respBinary(nonNullCnt) = 1;
        else
            p.respBianry(nonNullCnt) = 0;
        end
            
        if p.null(t)
            disp(sprintf('Trial %03i: Null',t));
        elseif p.cond == 3 % WM
            nonNullCnt = nonNullCnt + 1;
    ...
    if p.cond == 3
        ...
        p.accuracy = sum(p.respBinary  == p.validTarget);
