...
try
    ...
    block_p_points = FractDotRGSlow(prac1, win, color, p.decision, block_p_points); %Large example
    DrawCenteredNum(fix, win, color, p.fixation);
    ConsiderSlow(prac2, win, color, task, p.consider); %No answer example
    DrawCenteredNum(fix, win, color, p.fixation);
    ConsiderSlow(prac3, win, color, task, p.consider);
    block_p_points = FractDotRGSlow(prac3, win, color, p.decision, block_p_points); % Small example
    ...
    block_p_points = FractDotRGSlow(prac4, win, color, p.decision, block_p_points); %decision
    DrawCenteredNum(fix, win, color, p.fixation);%fixation
    ConsiderSlow(prac5, win, color, task, p.consider);%consider
    block_p_points = FractDotRGSlow(prac5, win, color, p.decision, block_p_points); %decision
    DrawCenteredNum(fix, win, color, p.fixation);%fixation
    ConsiderSlow(prac6, win, color, task, p.consider);%consider
    block_p_points = FractDotRGSlow(prac6, win, color, p.decision, block_p_points); %decision
    DrawCenteredNum(fix, win, color, p.fixation);%fixation
    ConsiderSlow(prac7, win, color, task, p.consider);%consider
    DrawCenteredNum(fix, win, color, p.fixation);%fixation
    ConsiderSlow(prac8, win, color, task, p.consider);%consider
    block_p_points = FractDotRGSlow(prac8, win, color, p.decision, block_p_points); %decision
    ...
    for kk = 1:p.runs;
        ...
        for ii = 1:(p.nRepeats/p.runs);
            ...
            for jj = 1:p.nStim;
                ...
                p.dotsResults(trialNbr+1,4:11,kk) = FractDotRGSlow_Abs(Test3DDots(trialNbr,:,kk), win, color, end_decision, p.dotsResults{trialNbr+1,18,kk}, points, LR);
        ...
        p.time_Runs(kk+1,4) = {end_t};
        block_points = points - block_points;
        DrawCenteredNum(fix, win, color, 1);%fixation to pad end of scan
        DisplayInstructs2;
    end
